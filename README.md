...
1. **Storage is a capital trap** — highest inventory value tied up, thinnest profit margins relative to CPUs. Renegotiate supplier contracts or shift assortment toward higher-margin SKUs.

2. **Video Cards carry the highest obsolescence risk** — large capital locked in fast-deprecating hardware. Promotional pricing on slow movers is urgent before the next technology cycle.

3. **Top 20% of products drive 80%+ of inventory value** — Pareto holds. These Class A products need dedicated replenishment SLAs and supplier priority agreements.

4. **15% of SKU-locations are Critical Low (≤20 units)** — silent stockout risk on potentially high-margin items. A safety-stock policy is overdue.

5. **US warehouses (San Francisco + Seattle) are disproportionately concentrated** — redistribute toward Asia-Pacific to reduce logistics risk and improve regional service levels.

6. **Price and profit per unit are strongly correlated but margin % is not** — expensive products are not always the most profitable by percentage. Margin management requires per-SKU analysis.
